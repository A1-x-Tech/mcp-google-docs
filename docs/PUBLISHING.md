...
(`io.github.A1-x-Tech/mcp-google-docs`). Check before publishing — all three must
...
2. `npm publish` — runs `typecheck` + `test` + `build` via `prepublishOnly` / `prepare`.
   The package is scoped (`@a1-x-tech/mcp-google-docs`) with
   `publishConfig.access: "public"`, so no `--access` flag is needed.
...
2. Point it at `https://github.com/A1-x-Tech/mcp-google-docs`. LobeHub pulls the README,
   tool list and install config (`npx -y @a1-x-tech/mcp-google-docs`) automatically.
