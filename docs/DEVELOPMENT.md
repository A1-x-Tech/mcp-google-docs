...
GOOGLE_DOCS_ACCESS_TOKEN=$(gcloud auth print-access-token) node dist/index.js
# optional: GOOGLE_DOCS_API_BASE, GOOGLE_DOCS_DRIVE_API_BASE, GOOGLE_DOCS_TIMEOUT_MS, GOOGLE_DOCS_MAX_RETRIES
```

## Live smoke

`npm run smoke` is READ-ONLY by default: with a document id (first argv or
`GOOGLE_DOCS_SMOKE_DOCUMENT_ID`) it fetches that document; without one it just mints an
access token from the refresh token — either way nothing is written.

`GOOGLE_DOCS_SMOKE_LIVE_WRITE=1 npm run smoke` opts into the full write scenario on a
**disposable** document: create → insert text → read back → export Markdown, then the
document is deleted in a `finally` block, so the cleanup runs after success and failure
alike. It never touches an existing document.
...
asserting the server identity, the instructions and the full tool list — both configured
and with no credentials at all (the degraded-start contract). Put a `*.test.ts` next to the
code it covers; `npm run typecheck && npm test` is the gate (also run by `prepublishOnly`).
...
(`~/.config/mcp-google-docs/instance-id`), the package version, the AI client's name and
version from the MCP handshake, the Node.js version and the OS.

OAuth credentials, document content, tool arguments and prompts are never sent or stored
