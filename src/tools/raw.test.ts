...
test("raw_request defaults to GET with the Bearer token", async () => {
  ...
  try {
    const res = await tools.raw_request({ path: "v1/documents/abc" });
    assert.equal(res.isError, undefined);
    assert.equal(calls[0].method, "GET");
    assert.equal(calls[0].url, "https://docs.googleapis.com/v1/documents/abc");
...
test("raw_request POSTs a JSON body to a relative path", async () => {
  ...
  try {
    await tools.raw_request({
      path: "v1/documents/abc:batchUpdate",
      method: "POST",
      body: { requests: [{ mergeTableCells: { tableRange: {} } }] },
    });
    assert.equal(calls[0].method, "POST");
    assert.equal(calls[0].url, "https://docs.googleapis.com/v1/documents/abc:batchUpdate");
    assert.deepEqual(calls[0].body, { requests: [{ mergeTableCells: { tableRange: {} } }] });
  } finally {
    restore();
  }
});
