...
function withEnv(vars: Record<string, string | undefined>, run: () => void): void {
  const keys = [
    ...
    "GOOGLE_DOCS_DRIVE_API_BASE",
    "GOOGLE_DOCS_TIMEOUT_MS",
    "GOOGLE_DOCS_MAX_RETRIES",
    ...Object.keys(vars),
  ];
...
test("no credentials at all is not an error — the config loads with empty fields", () => {
  withEnv({}, () => {
    ...
    assert.equal(config.apiBase, "https://docs.googleapis.com");
...
test("a partial OAuth triple reports incomplete_oauth_config", () => {
  assert.equal(reasonOf({ GOOGLE_DOCS_CLIENT_ID: "id" }), "incomplete_oauth_config");
  assert.equal(
    reasonOf({ GOOGLE_DOCS_CLIENT_ID: "id", GOOGLE_DOCS_CLIENT_SECRET: "secret" }),
  ...
  assert.equal(
    reasonOf({ GOOGLE_DOCS_REFRESH_TOKEN: "rt", GOOGLE_DOCS_ACCESS_TOKEN: "at" }),
    "incomplete_oauth_config",
  );
});
...
test("a static access token alone is enough", () => {
  withEnv({ GOOGLE_DOCS_ACCESS_TOKEN: "at" }, () => {
    const config = loadConfig();
...
test("base overrides are honored", () => {
  withEnv(
    {
      GOOGLE_DOCS_ACCESS_TOKEN: "at",
      GOOGLE_DOCS_API_BASE: "https://docs.example.test",
      GOOGLE_DOCS_DRIVE_API_BASE: "https://drive.example.test",
    },
    () => {
      const config = loadConfig();
      assert.equal(config.apiBase, "https://docs.example.test");
...
test("invalid numeric overrides fall back to the defaults", () => {
  withEnv(
    {
      GOOGLE_DOCS_ACCESS_TOKEN: "at",
      GOOGLE_DOCS_TIMEOUT_MS: "not-a-number",
      GOOGLE_DOCS_MAX_RETRIES: "-5",
    },
    () => {
      const config = loadConfig();
...
test("numeric overrides are honored when valid", () => {
  withEnv(
    { GOOGLE_DOCS_ACCESS_TOKEN: "at", GOOGLE_DOCS_TIMEOUT_MS: "1000", GOOGLE_DOCS_MAX_RETRIES: "0" },
    () => {
      const config = loadConfig();
